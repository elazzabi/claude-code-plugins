...
def _load_agent_reviewed_files(
    ...
    `reconciliation_context.load_agent_reviews` reads the same files for
    the findings payload through its own path and failure policy — it
    reports malformed output on stderr and skips it, which is not what
    run-level file review wants.
    """
    data = _load_final_review(output_dir, agent)
    if data is None:
        return None
...
def aggregate_file_review(
    ...
    Lives beside `build_assignment_manifest`, which answers the same
    question — which changed files no agent's scope contained — from
    dispatch-time evidence over a different population. See that
    builder's DIVERGENCE NOTE before "reconciling" the two numbers.
    """
    inline: Dict[str, set] = {}
    ...
    return {
        ...
        # the same measurement as the run manifest's
        # `assignment.unassigned_reviewable_files`: different population
        # (full changed set vs. noise-filtered `reviewable`) over
        # different evidence (runtime sidecars vs. dispatch-time SCOPE
        # events), so the two numbers legitimately differ. The full
...
def build_assignment_manifest(
    ...
    try:
        ...
        return {
            ...
            # publishes into the run's `file_review`, and the two
            # legitimately disagree (a field
