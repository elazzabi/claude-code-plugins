...
- The orchestrator's hints reach the reconciliator as claims it must answer with an outcome and evidence, and the record renders the answers under `Orchestrator Notes` beside `Dropped by the Reconciliator`. Claims registered against a run survive a step-8 rebuild, so a retry after an interrupted dispatch cannot quietly release the requirement that each be answered.
...
- **`--refresh-deps`** — opt-in interactive dependency refresh runs only after a clean tracked-worktree check, then publishes a schema-validated report through one atomic save command. Defaults off and stays disabled for bots.
...
- **Token-usage snapshot** — `usage-snapshot.json` captures a run's token cost at finalize and reprojects it into the manifest and `pipeline-result.json`; re-runs update monotonically, so weaker evidence never overwrites good.
- **Synthesis-agent lifecycle** — the reconciliator and decision critic get dispatch markers and completion-derived durations (`synthesis-agents.json` and the manifest).
...
- **`review-record.md`** — a machine projection of the findings ledger carrying findings, verified checks, assessment, critic accounting, run notes, and the coverage measurement in one place. No agent writes or edits it, so it cannot drift from the ledger it projects.
...
- **Budget pressure moved to the save echo** — the unenforceable under-budget rule is gone, and the target is echoed back whenever review files remain unclaimed. The builder now reads every assignment fact — claimable files, budget, channels — from the schema-5 assignment in its bound output directory instead of environment variables. The assignment carries the inline file set itself, so claiming an inline file is a harmless no-op rather than a refused call.
...
- **`review-report.md` is authored at step 11 from the settled post-critic record** instead of being drafted at step 9 and edited after every critic REVISE. A source fingerprint rejects stale prose, and `pipeline-result.json` publishes only once the report is bound to unchanged settlement state.
- **The final review verdict is derived from the findings ledger** — the orchestrator no longer transcribes one and `review-verdict.json` is gone, with `pipeline-result.json` recording the branch under `verdict_source`. A critic ESCALATE still overrides the published verdict to COMMENT.
...
- **The decision critic saves every verdict through a validating script channel** — `critic.py --save` validates the whole batch and records findings, a digest-bound proposal, and the STAND, REVISE, or ESCALATE verdict together. An interrupted publication stays visibly incomplete rather than half-committed.
...
- **The run-level file review is complete and honest** — `unscoped_files` stays `null` when unmeasured, the scope summary preserves the every-mode routing population even when no diff is fetched, and a reviewer that never finalized is not recorded as both receiving and never reviewing a file its domains split between inline and claimable. Both sides of the comparison share one path grammar.
...
- **A dispatched critic that produced no usable verdict degrades the run** — including one recorded as SKIPPED after the fact, which describes the same lost stress test. Quick mode's own skip is written by the pipeline and stays silent.
- **The reconciled ledger carries what held** — the reconciliator records checks surviving its universal method judgment, and step 9 sources "what was verified and held" from the ledger, never memory.
- **The transcript analyzer stopped inventing missing evidence** — results it mines nothing from (WebSearch, WebFetch, MCP) resolve on pairing, and unresolved is reserved for genuine record damage. `subagents: complete` is earnable again.
...
- **`scope_reporting_agent_count` counts agents, not scope-summary files.**
- **Briefing no longer instructs bulk enumeration of unclaimed review files** — the builder derives the complement from reviewed-file claims, freeing that budget for real analysis instead of bookkeeping.
- **Measurement honesty sweep** — damaged JSONL records are counted, unmeasured never publishes as zero, availability flags derive from what was actually parsed, and a builder save is recognized by its envelope on any line of the command rather than only as a first-line heredoc. A coverage-manifest build failure is now distinguishable from legitimate absence.
...
- **Automatic dependency installation from the reviewed repo's manifests.** Package managers execute PR-controlled configuration as code (`.pnpmfile.cjs` survives `--ignore-scripts`; Composer `path` repositories read outside the checkout), so `ensure_installed.py`, the `hosts/install/` package, and the install-cache resolver are gone, and missing dependency source now reports honestly as a degraded host-context banner instead of being manufactured at install time.
- **The per-clone library-dep cache** — `~/.cache/pirategoat/library-deps/` is no longer read and can be deleted. Dependencies already installed in the clone and the machine-wide WordPress/WooCommerce ecosystem cache remain the trusted paths.

### Fixed

- **Manifest status is the single completeness authority for outcomes** — running manifests with numeric summary totals now cap raw/final outcomes at partial, matching the coverage gate.
- **Open-ended run windows close at the next human turn** — a crashed run's window no longer absorbs every later unrelated turn in the session, and synthetic user records don't count as human prompts.
- **Stage attribution requires same-run identity** — step events from another run invalidate the timeline instead of shifting main-session tokens into foreign stage boundaries.
- **Heredoc reconstruction binds issues to the saved receiver** — a two-builder heredoc no longer merges one builder's unsaved issues into the other's saved record, and saves through anything but a plain variable fail closed.
- **Save dedup canonicalizes artifact paths**, so `/out/x.json` and `/out/./x.json` count as one artifact under last-save-wins.
- **Running lifecycle sidecars validate their incomplete-sets at any status** — a sidecar understating incompleteness is damaged evidence and fails closed.
- **Legacy reconstruction is frozen as best-effort inference** — new inference-precision edges are known limitations rather than fix rounds, unless they crash, breach privacy, or contaminate non-legacy evidence.
- **Each Codex repo reviewer is its own task** — task names now derive from the reviewer instance at both steps 6 and 8, so a second repo reviewer no longer collides on the shared adapter name.
- **Generated Codex skills translate the Claude-only session variable** — `CLAUDE_SESSION_ID` becomes `CODEX_THREAD_ID` at the host seam, so Codex reviews stop correlating to an empty session.
- **One execution, one model tier** — the dispatch plan records the host-aware effective tier (`inherit` under Codex) with the Claude declaration preserved as `declared_model`.

*The full narrative for this release — rationale, evidence, and field-run numbers — lives in the commit bodies.*

## [1.112.0] - 2026-07-29

Makes the review pipeline measurable — durable telemetry manifests plus a supported run/cohort metrics interface — and puts the resulting pressure on reviewers to spend the budget they are given on the NOT DIFFED queue.

### Added

- **NOT DIFFED files are a work queue, not an appendix** — the scope section states the files ARE in scope, and the budget briefing directs unspent budget at them largest-first; finishing early with in-scope files unread is a coverage gap, not efficiency.
- **Declaring a file unreviewed requires genuine budget exhaustion** — an APPROVE that silently ignores NOT DIFFED files, or a declaration written with most of the budget unspent, is a protocol violation, and utilisation-vs-target is measurable per run.
- **`ReviewOutputBuilder.add_unreviewed(file)`** gives budget omissions a supported representation — declared paths surface as `unreviewed` in the JSON and render the mandated `**Not reviewed (budget):**` line, without affecting the verdict.
- **Review runs emit durable measurement manifests** — versioned schema, unique run ID, session/plugin/repository/range identity, step and agent lifecycle, aggregate outcomes, and explicit availability metadata, without retaining PR, prompt, finding, or tool-result prose.
- **Planner decisions stay measurable after orchestration** — step 5 preserves an immutable deterministic dispatch baseline, and manifests compare it against the orchestrator-adjusted plan.
- **Run manifests derive changed-file coverage** — assigned, excluded, and uncovered path sets from dispatched reviewer scopes, labeled descriptive rather than proof of model reads.
- **Claude session transcripts can enrich run measurements** — cache-aware token usage, tool failure and recovery, first pipeline-owned Bash attempts, and observed repository reads, correlated to the exact session and subagents without retaining review prose.
- **`scripts/analysis/review_run_metrics.py` is the one supported run/cohort measurement interface** — it prefers durable manifests, safely reduces legacy JSONL, and reports every metric family as complete/partial/missing/disabled instead of zero-filling; transcript enrichment applies only to bounded queries (`--last`, `--run-id`).
- **Path-declared applicability participates in reviewer scope** — a repo reviewer matched by `applies_to.paths` now receives those files via scope.py's new `--include-path` glob axis, so the dispatch gate and the scope never disagree.

### Security

- **Repo reviewer prompts execute only with merged provenance** — any rule or reviewer whose defining file (or `.pirategoat/config.json` itself) changed in the reviewed range is hard-excluded, never dispatchable, and surfaced as a step-5 warning, closing the `pull_request_target`-style hole where a PR handed the review session its own instructions.
- **The provenance gate matches every spelling of a changed path** — Git C-quoted names, symlink-resolved targets, casefolded/NFC canonical identities, and changed gitlinks tainting everything beneath them — so no encoding, in-repo link, case variant, or submodule update slips PR-controlled text past it.
- **Advisory verdict suppression requires declared entitlement** — the builder accepts only the exact `blocking`/`advisory` vocabulary, entitlement is enforced at add time and serialization, and review JSON and manifests record the suppression count plus the stricter verdict over all findings when suppression softened it.
- **An explicit isolation request never widens into inline execution** — `execution: "isolated"` is refused at dispatch and bootstrap instead of silently falling back to the most permissive mode.
- **Dependency roots must resolve inside the reviewed repo** — an in-repo symlink pointing at an external directory can no longer make that directory an install root.
- **Repo-supplied globs can no longer stall the pipeline** — `glob_match` is a non-backtracking dynamic program with identical glob semantics.

### Changed

- **The review JSON is the single published reviewer artifact** — Markdown is a pure render of it (`python3 output.py render|materialize`), so a mismatched JSON/Markdown pair is unrepresentable and every machine consumer keys on the JSON alone.
- **The hosts containment invariant is enforced at one shared point**, with a drift guard banning inline containment re-derivations under `scripts/hosts/`.
- **The mirrored timestamp parsers are byte-for-byte aligned**, so a boundary timestamp can no longer be valid evidence in one module and a gap in the other.

### Fixed

- **Mandatory NOT DIFFED handling now actually reaches reviewers** — 1.108.0 wrote the rule into a protocol section bootstrap strips before any agent sees it; the contract now travels in the REVIEW BUDGET briefing, guarded by a regression test.
- **Budget sizing and telemetry scope count the whole workload** — NOT DIFFED and list-only files enter budget line counts, telemetry scope paths, and coverage, so the largest reviews stop computing the smallest targets and deferred-queue reads stop classifying as out-of-scope; bootstrap consumes machine-readable scope sidecars instead of re-parsing its own rendered text.
- **Capped budgets no longer claim calibration** — when the 80-call clamp is hit, the briefing presents the target as an effort floor rather than proof of coverage.
- **Budget utilization has its numerator** — agent usage carries `tool_calls` beside agent-start `budget_target`, so declared budget exhaustion with calls remaining is auditable.
- **Unreviewed declarations are validated at both ends** — the builder rejects paths outside the reviewer's own persisted deferred set, coverage re-verifies them consumer-side, malformed lists claim nothing, and paths canonicalize, so a typo or malformed field can never invert into a full-review claim.
- **Coverage reconciles deferred-file outcomes** — deferred files an agent's output accounts for report as `files_deferred_reviewed` (agent claim, not proof of read), declarations surface as `files_declared_unreviewed`, and remaining genuine gaps stay annotated.
- **Step 1 clears every per-run artifact** — stale telemetry log paths, `.started` markers, reconciliation context, scope summaries, session IDs, and reusable output-directory context can no longer contaminate a fresh run's telemetry, status, or coverage.
- **Durable run identity stores the reviewed commit SHAs** — symbolic refs resolve (annotated tags peeled, three-dot ranges parsed correctly), interactive PR runs record the post-checkout head, and manifest refreshes never overwrite resolved endpoints with movable refs.
- **Reviewer saves are atomic and race-free** — each save stages under a unique nonce and completion telemetry logs inside the publication lock before the readiness artifact appears, so overlapping retries can neither corrupt output nor let a racing finalize record a finished agent incomplete.
- **Bootstrap imports cleanly on Python 3.10-3.13** — an unimported annotation name killed every bootstrap invocation on the supported older interpreters, and a suite-wide guard now forces annotation evaluation so the 3.14 test suite fails where 3.10 would.
- **Parallel reviewers no longer create shared temporary builder scripts** — bootstrap is the sole source of the collision-safe one-shot heredoc, ending the first-write failures that hit 30 of 139 historical reviewer runs and giving measurement one canonical builder envelope to recognize.
- **The semantic diff filter exempts prose and path-rescued files** — Markdown syntax matched its code-comment heuristics, so docs-domain and path-rescued diffs were stripped and reviewers could report clean without seeing the edit that triggered dispatch.
- **Repo-reviewer scope discovery failures fail loudly** — when every declared domain errors, the adapter reports STATUS: ERROR and exits 1 instead of producing a clean not-applicable result for a run that never inspected anything.
- **Adapter ref-mode instances carry their own identity end to end** — telemetry, sidecars, scope summaries, coverage, and review-file stems key on the per-instance name (only the trailing `-reviewer` stripped), instances receive the validated absolute prompt path, and IDs are restricted to the lowercase-kebab measurement contract, so N repo reviewers never collide or vanish from measurement.
- **Repo rules reach the reviewers they target** — selection uses the effective instance identity, declared domains, and the complete in-scope path set, and advisory rules carry an explicit channel contract so they cannot gate the verdict through untagged findings.
- **Dispatched model tiers are recorded honestly** — Codex-hosted dispatches stop recording a Claude tier that never ran, and repo-reviewer model overrides reach dispatch and lifecycle telemetry consistently.
- **Dependency roots resolve from the review scope, not just the repo root** — each changed file (Git-quoted paths included) contributes its nearest lockfile-bearing ancestor, capped per manager with the remainder surfaced through the degradation banner (`dep_roots_capped`, added to the TypeScript banner type) instead of silently narrowed.
- **JS installs stage every input the install reads** — manager config, declared patches, and workspace member manifests — and cache freshness keys on that exact staged set, so pnpm workspaces install instead of failing and a config-only change can no longer serve a stale dependency layout.
- **Installs can never dirty or confuse the reviewed worktree** — Composer cache and bin dirs redirect into the cache slot, staged symlinked inputs keep their declared paths, nested roots get collision-proof slot and host-context identities, and only the current review's slots reach host context.
- **Measurement never reports missing evidence as a measured zero** — usage-less transcripts, damaged JSONL lines, unresolved or duplicate tool calls, unclassifiable results, and absent correlation counts degrade exactly the affected family to partial or missing instead of entering complete denominators.
- **Every measured window and segment is bounded to its own run** — windows anchor on the run's triggering prompt through its final presentation turn, synthetic harness records are ignored, subagent transcripts are run-window bounded, and legacy log segments cut at the first foreign or terminal event, so one run can never absorb another's usage, outcomes, or lifecycle.
- **Tool-result classification matches real transcripts** — validated success shapes beat prose failure signatures, non-text Read variants and legacy Grep/Glob shapes count as the successes they are, and last-wins usage records end the undercounting of split assistant responses.
- **Measurement projections validate their own completeness** — one shared dispatch-status vocabulary and agent-identity grammar on every surface, canonical repository-relative path validation, fail-closed ingestion of malformed sidecars and statuses, producer-identity latest-wins critic-skip selection, exact separation of reviewer, synthesis, and orchestrator evidence families, and contract modules loaded by exact path.
- **Session quality analysis reconstructs the mandated Bash builder** — review records come from the heredoc's literal `add_issue()` calls, counting only what a confirmed save actually persisted (severity floors honored, control flow fails closed, unrelated JSON writes ignored) instead of producing empty or fabricated per-agent records.
- **Fail-open robustness at the edges** — over-long telemetry filenames shorten deterministically instead of silently costing a run its telemetry, a non-object `review-context.json` degrades to empty context instead of crashing, and the Codex generator skips gitignored dotfiles instead of crashing on a `.DS_Store`.
